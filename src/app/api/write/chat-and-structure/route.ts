...
export async function POST(req: Request) {
  try {
    ...
    const processPromises = inputParagraphs.map(async (para: string, index: number) => {
      ...
      try {
        const structurePrompt = `你試試寫這一段，寫法是先想像兩個人物對談，再將資訊加工，整理成適合 SEO 的內容格式（問題陳述，嚴重性，解決方案）。
輸出對談與整理結果
對話的是兩個台灣人
...
重點：請確保最後『對話內容整理』的用詞與邏輯順序，能反映（或呼應）前方『對話內容』的鋪陳。讓整理結果看起來像是從人物對話中直接提煉的重點，而不是簡短隨意的內容。
重點2：整理中出現列表格式是給已經看完對話內容的人查閱的，需要在前後放置對話內容的整理小段落，幫助理解情境
重點3：整理對話，主要是去掉回應詞，例如，是的，對的，等回應，但是內容主體可以直接使用到整理中，以利閱讀。

整理對話直接寫出整理好的，刪掉描述寫作策略的句子

且用詞不要有偏好，使用中性用詞，使用一般人的用詞
要更多使用對話中的內容，整理不是簡短清單，而是通俗易懂，完整段落。
...
注意：
問題陳述，嚴重性，解決方案 不是要你描述 SEO 問題，而是要使用聊天內容

輸出的對話內容整理，需包含在三個區塊內：問題陳述，嚴重性，解決方案。不需要額外區塊
