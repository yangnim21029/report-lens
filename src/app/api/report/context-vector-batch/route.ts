...
interface BatchResult {
  pageUrl: string;
  success: boolean;
  suggestions?: ContextVectorSuggestion[];
  markdown?: string;
  error?: string;
}
...
async function processItem(item: BatchItem) {
  const pageUrlRaw = item.pageUrl.trim();
  const analysisText = item.analysisText ?? "";
  const providedText = typeof item.articleText === "string" ? item.articleText.trim() : "";
  const providedHtml = typeof item.articleHtml === "string" ? item.articleHtml : "";
  ...
  if (!articlePlain) {
    if (!pageUrlRaw) {
      throw new Error("Missing pageUrl or article content");
    }
  ...
  if (!articlePlain) {
    throw new Error(
      `Unable to retrieve article content${captureError instanceof Error ? `: ${captureError.message}` : ""}`
    );
  }
  ...
  const response = await openai.responses.parse({
    ...
    console.warn("[context-vector-batch] parse error", err);
    return null;
  });
