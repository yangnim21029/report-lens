...
LIMIT 100;`;
	...
	// Get search data for a specific page URL (single page focus)
	getSearchDataByUrl: publicProcedure
		.input(
			z.object({
				site: z.string().default("sc-domain:holidaysmart.io"),
				page: z.string().url(),
		...
		.query(async ({ input }) => {
			// Reuse the main SQL with an extra page filter and without the 50-clicks threshold
			const sql = `-- 日期設定區
WITH date_settings AS (
    SELECT 
        CURRENT_DATE - INTERVAL '7 days' as current_period_start,
			...
			try {
				const response = await fetch(
					"https://unbiased-remarkably-arachnid.ngrok-free.app/api/query",
					{
						...
						body: JSON.stringify({
							site: input.site,
							sql: sql.replace('{page}', input.page.replace(/'/g, "''")),
						}),
					},
				);
				...
				const data = await response.json();
				let resultArray: SearchResult[] = [];
				if (Array.isArray(data)) resultArray = data as SearchResult[];
				else if (data?.data && Array.isArray(data.data)) resultArray = data.data as SearchResult[];
				else if (data?.results && Array.isArray(data.results)) resultArray = data.results as SearchResult[];
				else if (data?.rows && Array.isArray(data.rows)) resultArray = data.rows as SearchResult[];
				else resultArray = [];

				return resultArray;
			} catch (error) {
				console.error("Error fetching search data by url:", error);
				throw new Error("Failed to fetch search data by url");
			}
		}),
});
